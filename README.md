...
- **placement.c** : This file is used to perform data placement between Dram and Optane based on the profiling and placement results of our algorithm. It reads the output.txt file
  generated by our placement algorithm, and switches between the two memories after it has allocated as many bytes as it reads from the current line of the file, to the current
  memory type, starting with optane. For example, if the first three lines are:
  
 <img width="145" height="57" alt="Image" src="https://github.com/user-attachments/assets/e92bdd25-050b-4c42-8344-b798e14f9561" />
  
  this means that the algorithm must place the first 1 kB to optane, the next 2 kB after that to Dram,
  and the next 3 kB to Optane.
